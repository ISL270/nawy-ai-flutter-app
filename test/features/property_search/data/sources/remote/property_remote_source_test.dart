...
class TestPropertySearchRemoteSource {
  final PropertySearchResponse _testSearchResponse;

  TestPropertySearchRemoteSource(this._testSearchResponse);
...
void main() {
  group('PropertyRemoteSource Tests', () {
    late TestPropertySearchRemoteSource remoteSource;
    ...
    setUp(() {
      ...
      remoteSource = TestPropertySearchRemoteSource(testSearchResponse);
    });
    ...
    group('Data Consistency', () {
      ...
      test('should filter by text search query in property name, area, and compound', () async {
        ...
        expect(
          areaResult.properties.length,
          equals(2),
        ); // Properties 1 and 4 are in El Sheikh Zayed
      ...
      test('should handle complex combined filtering scenarios correctly', () async {
        ...
        expect(
          textAreaTypeResult.properties.length,
          equals(1),
        ); // Property 1: Luxury Apartment in ZED
        ...
        expect(
          textPriceResult.properties.length,
          equals(1),
        ); // Property 4: Spacious Villa (15M-18M)
        ...
        expect(
          textCompoundResult.properties.length,
          equals(1),
        ); // Property 2: Modern Villa in Bloomfields
        ...
        expect(
          allFiltersResult.properties.length,
          equals(2),
        ); // Properties 1 and 4 match all criteria
      ...
      test(
        'should maintain filter independence - text search should not override other filters',
        () async {
          ...
          expect(
            priceIndependentResult.properties.length,
            equals(1),
          ); // Property 2: Modern Villa (8M-12M max)
          expect(
            priceIndependentResult.properties.first.name,
            equals('Modern Villa in Bloomfields'),
          );
...
PropertySearchResponse _createTestSearchResponse() {
