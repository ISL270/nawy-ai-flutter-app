...
void main() {
  group('API Contract Tests', () {
    ...
    test('Properties Search API contract validation', () async {
      ...
      expect(() => SearchResponse.fromJson(data), returnsNormally);
    ...
    test('API field coverage validation', () async {
      ...
      if (properties.isNotEmpty) {
        ...
        final criticalFields = [
          'id', 'name', 'slug', 'property_type', 'compound', 'area', 'developer',
          'image', 'finishing', 'min_unit_area', 'max_unit_area', 'min_price',
          'max_price', 'currency', 'number_of_bedrooms', 'number_of_bathrooms'
        ];
        
        for (final field in criticalFields) {
          if (propertyJson.containsKey(field) && propertyJson[field] != null) {
            expect(serializedJson.containsKey(field), true, 
